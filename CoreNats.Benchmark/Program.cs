namespace SimpleBenchmark
{
    ...
    class Program
    {
        ...
        static async Task RunBenchmarkInner(int publishers,int subscribers,int messageSize,int msgPerSecond,bool writerGenerate=true, TimeSpan? duration=null,string subject=null)
        {
            ...
            async Task<(long count,long sum,long max,long min,double mean)> ReaderText(NatsConnection connection, string subject,CancellationToken cancellationToken)
            {
                ...
                try
                {
                    ...
                    await connection.SubscribeAsync(subject, Process, cancellationToken: cancellationToken);
